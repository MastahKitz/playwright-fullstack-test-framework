#!/usr/bin/env node
// Pick the one consistent code location for each failing/flaky test in a run.
//
...
// or the spec itself), which makes `file:line` an unstable key across runs. This
// picks a single deterministic anchor per failure — the DEEPEST stack frame that
// lives in a `*.{spec,flow,actions,assertions}.ts` file under `tests/functional/`
// — so the results-analysis workflow, the marker it commits, and every later run
// all key off the same line.
//
// Usage:
//   node scripts/extract-failure-anchors.js [test-report/results.json] > failure-anchors.json
...
function firstStack(test) {
  for (const result of test.results || []) {
    for (const err of result.errors || []) if (err.stack) return err.stack;
    if (result.error && result.error.stack) return result.error.stack;
  }
  return '';
}

...
export async function assertProductListSuccess(response: APIResponse) {
  ...
  // qademo's catalog is shared/live and grows over time; assert internal consistency instead of a fixed count.
  expect.soft(body.data.length, 'meta.total should match the number of products returned').toBe(body.meta.total);
}

...
// `username` is only known up front when the request supplied one — when it's
// omitted the server generates it, so an unmatched non-empty string is expected.
export async function assertSignupSuccess(
  response: APIResponse,
  expected: { username?: string; email: string; phone: string },
) {
...
export async function assertInvalidEmailError(response: APIResponse) {
  ...
  assertResponseBody(body, {
    ...
    error: {
      issues: [
        { validation: 'email', code: 'invalid_string', message: 'Please enter a valid email address', path: ['email'] },
      ],
      name: 'ZodError',
    },
  }, { exact: true });
...
export async function assertInvalidPhoneError(response: APIResponse) {
  assertResponseStatus(response, 400);
  const body: SignupValidationErrorResponseBody = await response.json();
  assertResponseBody(body, {
    success: false,
...
export async function assertBlankPasswordError(response: APIResponse) {
  assertResponseStatus(response, 400);
  const body: SignupValidationErrorResponseBody = await response.json();
  assertResponseBody(body, {
    success: false,

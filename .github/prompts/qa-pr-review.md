...
5. **Locators are testid-first.** Use `getByTestId(...)` (including regex testids like
...
6. **Assertions use `expect.soft(...)`**, not bare `expect(...)`, inside `.assertions.ts` files,
...
8. **Every `test.describe(...)` has a `{ tag: '@xxx' }`** consistent with its domain (existing
   tags: `@auth`, `@product`, `@cart`, `@checkout`).

9. **`test.describe.configure({ mode: 'serial' })`** is required when a suite's tests depend on
...
10. **Every click is followed by a deterministic wait.** One of:
...
12. **Test titles read as `'validate user can/cannot <do something>'`**, matching every existing
    spec.

13. **Base URL comes from `tests/functional/config/environments.ts`; credentials come from the
